defmodule OverviewSimulation do
  ...
  import Enum
	...
	@spec slotHonestMsgs(%OverviewSimulation{}, non_neg_integer(), 
		list(non_neg_integer()), list(Validator.msg()), list(Validator.msg())) 
			:: {%OverviewSimulation{}, list(Validator.msg()), list(Validator.msg())}
	def slotHonestMsgs(config, t, validatorIds, msgsOutPart1, msgsOutPart2) do
		case validatorIds do
			...
			[validatorId | tail] ->
				...
				slotHonestMsgs(config, t, tail, msgsOutPart1, msgsOutPart2)
		end
	end
	...
	def log_ledger_lengths(config, t) do
		...
		l_Lda_adv = Enum.min(config.validatorsAdversarial |> Enum.map(fn x -> List.length(AdversarialValidator.lda(x))-1 end))
